...
void Sequence::CompareAndRemove(GenomeSequenceInfo &FSequences,
  ...
  if (!IsContainsFLH(RFSequences, RHSequences, RLSequences)) {
    ifs.close();
    return;
  }
      ...
      for (size_t i = 0; i < bpMatrix.size(); i++) {
        if (mutationPointCount[i] >= 10) {
          ...
          if (RLSequences.Sequences.size() == 1 &&
              RFSequences.Sequences.size() + RHSequences.Sequences.size() ==
              i) {
            mutationPointCount[i] = 0;
            continue;
          }
...
bool Sequence::IsContainsFLH(GenomeSequenceInfo &RFSequences,
                             GenomeSequenceInfo &RHSequences,
                             GenomeSequenceInfo &RLSequences) {
  if (RFSequences.Sequences.size() < 1 || RHSequences.Sequences.size() < 1
      || RLSequences.Sequences.size() < 1) {
    return false;
  }
